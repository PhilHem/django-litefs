...
>
> **Target**: Keep this file under 300 lines. Currently ~670 lines — extraction needed.
>
> **Suggest extraction** when you spot:
>
> - Patterns reusable across multiple projects → global skill
> - Project-specific but rarely-changing reference material → `.claude/docs/`
> - Sections over 50 lines of static documentation → separate file with reference
>
> **Before extracting, always**:
>
> 1. **Read the target file first** (if it exists) to understand existing content
> 2. **Deduplicate**: Merge new knowledge with existing, don't create duplicates
> 3. **Improve**: If existing content is weaker, enhance it; if new content is weaker, discard it
> 4. **Consolidate**: Combine related concepts rather than creating parallel sections
> 5. **Update references**: Ensure all files pointing to the target reflect the merged content
>
> **Knowledge evolution**: Skills and docs should grow stronger over time, not larger. Prefer refining existing knowledge over adding new sections.
>
> **Reference pattern**: Instead of inlining details, use:
>
> ```markdown
> See [.claude/docs/ARCHITECTURE.md] for layer diagrams and dependency rules.
> ```

## Extraction Candidates

> These sections should be extracted in future refactoring sessions:

| Section                                        | Target Location                                | Reason                                    |
| ---------------------------------------------- | ---------------------------------------------- | ----------------------------------------- |
| "Clean Architecture" + "Architectural Charter" | `~/.claude/skills/clean-architecture/SKILL.md` | General pattern, reusable across projects |
| "Package Structure (Clean Architecture)"       | `.claude/docs/ARCHITECTURE.md`                 | Static reference, rarely changes          |
| "Target Deployment Architecture"               | `.claude/docs/DEPLOYMENT.md`                   | Operational reference                     |
| "Answered Questions" + "Project Decisions"     | `.claude/docs/DECISIONS.md`                    | ADR/decision log                          |
| "Roadmap" + "Open Questions"                   | `.claude/docs/ROADMAP.md`                      | Planning artifact                         |
| "Testing Strategy" (full section)              | `.claude/skills/litefs-testing/SKILL.md`       | Project-specific skill                    |
| "Settings Interface" examples                  | `.claude/docs/CONFIGURATION.md`                | Reference material                        |
| "Dependencies" tables                          | `.claude/docs/DEPENDENCIES.md`                 | Static reference                          |

## Reference Index (Future)

> Once extracted, these references will point to detail files. Read on demand, not by default.

| Topic                          | Location                                       | Status               |
| ------------------------------ | ---------------------------------------------- | -------------------- |
| Architecture layers & diagrams | `.claude/docs/ARCHITECTURE.md`                 | 🔴 Not yet extracted |
| Design decisions & ADRs        | `.claude/docs/DECISIONS.md`                    | 🔴 Not yet extracted |
| Deployment & networking        | `.claude/docs/DEPLOYMENT.md`                   | 🔴 Not yet extracted |
| Testing patterns & fixtures    | `.claude/skills/litefs-testing/SKILL.md`       | 🔴 Not yet extracted |
| Clean Architecture rules       | `~/.claude/skills/clean-architecture/SKILL.md` | 🔴 Not yet extracted |
