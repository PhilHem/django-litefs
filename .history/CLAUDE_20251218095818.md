...
- **Primary Platform**: Self-hosted (Docker Compose, VMs, bare metal with Consul)
- **Secondary Platform**: Fly.io (if integration is trivial)
...
LITEFS = {
    ...
    "CONSUL_URL": None,                # Consul URL for leader election (primary method)
    "LEASE_URL": None,                 # Fly.io lease endpoint (optional, if trivial)
...
- [ ] Should we support non-Fly.io Consul setups? (leaning yes)
